...
The threat landscape itself is shifting in ways that make bridging more necessary, not less. Research on malicious AI swarms shows that state‑level polarisation attacks increasingly use *true* information — real news snippets, genuine statistics, authentic quotes — amplified with strong emotional framing. Every claim is factually correct; the attack lies in the curation, not the content. Debunking cannot touch this, because there is nothing false to debunk. But bridging can, because it surfaces the *overlap* that curated outrage is designed to hide. Taiwan's COVID experience demonstrated this concretely: when opposing camps each held partial scientific truths about mask efficacy — N95‑only advocates and mask‑sceptics both citing real studies — debunking either side only fuelled the fight. A pre‑bunking campaign built on humour (a Shiba Inu reminding people to keep their hands from their faces) went viral, depolarised the conversation, and increased handwashing — without declaring either side wrong.
...
The defensive response — evaluating models against pillars of data security, alignment, safeguard robustness, and development transparency — is necessary. The 6‑Pack's principles are structurally compatible with it:

**Data sovereignty** is not an add‑on; it is load‑bearing infrastructure. Pack 5's portability and meronymity, Pack 6's subsidiarity, and the insistence on local models running on local hardware are direct defences against the data exfiltration risk. When a bridging facilitator runs on commodity hardware in a government office, there is no API call to a jurisdiction that compels disclosure. The communitarian case for local compute is also the security case.

**Political bias** in an adversarial model is a specific instance of the general alignment problem. The 6‑Pack's answer is not simply "use a democratic model instead" — because any model trained without community input will have blind spots. Anthropic's own alignment assembly found that Claude assumed all users could walk, because the development team included no wheelchair users. The structural fix is not switching vendors; it is the alignment assembly itself, applied to whatever model a community adopts.

**Backdoors and sleeper agents** are hardest to detect in monolithic systems that concentrate trust. The kami architecture — many small, bounded, purpose‑specific models — limits blast radius by design. A compromised translation model cannot exfiltrate defence procurement data if it has no access to it. Pack 3's corrigibility (the model accepts shutdown) and Pack 4's community‑authored evaluations provide ongoing, distributed detection that no single red team can replicate at scale.
