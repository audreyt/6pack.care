...
There is also a basic misconception about the compute required. Civic AI does not need a general‑purpose model that memorises the entire internet. A bridging facilitator needs to translate "climate justice" into "biblical creation care" for a table of ten — not generate Studio Ghibli animations. Small, purpose‑specific models hallucinate less and perform better at facilitation tasks. They run locally, on commodity hardware, using a thousand to ten thousand times less energy than training a frontier model. When the model is local, data stays local too — no feudal cloud dependency, no perverse incentive to monetise deliberation data. The cost objection assumes civic AI requires frontier scale. It does not.
