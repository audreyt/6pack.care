...
For Civic AI, the public question is simple: when a system harms someone, who can contest the result, how fast, and what changes because they spoke?
...
- **Community-authored evaluations.** Affected communities co-write the tests for harm and successful repair. Some teams later feed those signals into routing or model updates — sometimes called Reinforcement Learning from Community Feedback (RLCF) — but that implementation choice is secondary. The public commitment comes first: the community authors the yardstick.
- **Shared eval registry.** Use a public, Wikipedia-like registry for tests: anyone can draft an eval, civil society partners review it, and labs adopt it or explain why not. These are living, community-maintained test suites, not lab benchmarks vendors can quietly teach to the test.
...
2. **Accept harm drafts.** Let people submit proposed evals in plain words; convert them into community-reviewed tests with partners.
3. **Triage by severity.** Task POs with classifying by the community's severity scale; the highest classes trigger immediate pause or reversible defaults.
...
- **Community feedback pipeline.** Turn approved evals into routing, training, or policy signals when appropriate, including RLCF where it genuinely fits.
